...
def logscreen():
    ...
    cmp1 = cpm
    cpm = 0
    utils.set_title(f"FOXLEN by KillinMachine | Checked: {checked}\{len(Combos)}  -  Hits: {hits}  -  Bad: {bad}  -  2FA: {twofa}  -  SFA: {sfa}  -  MFA: {mfa}  -  Xbox Game Pass: {xgp}  -  Xbox Game Pass Ultimate: {xgpu}  -  Valid Mail: {vm}  -  Other: {other}  -  Cpm: {cmp1*60}  -  Retries: {retries}  -  Errors: {errors}")
    time.sleep(1)
...
def Main():
    ...
    utils.set_title("FOXLEN by KillinMachine")
    os.system('cls')
    try:
        loadconfig()
    except:
        print(Fore.RED+"There was an error loading the config. Perhaps you're using an older config? If so please delete the old config and reopen FOXLEN.")
        input()
        exit()
    print(logo)
    try:
